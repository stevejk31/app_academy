...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes AS b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops AS stops_a ON a.stop_id = stops_a.id
    JOIN
      stops AS stops_b ON b.stop_id = stops_b.id
    WHERE
      stops_a.name LIKE 'Craiglockhart'
    AND
      stops_b.name LIKE 'Tollcross'
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    FROM
      stops AS stops_a
    JOIN
      routes AS routes_a ON stops_a.id = routes_a.stop_id
    JOIN
      stops AS stops_mid ON stops_mid.id = routes_a.stop_id
    JOIN
      routes AS routes_b ON routes_b.stop_id = stops_mid.id
    JOIN
      stops AS stops_b ON routes_b.stop_id = stops_b.id
    WHERE
      stops_a.name LIKE 'Craiglockhart'
    AND
      stops_mid.name NOT LIKE 'Craiglockhart'
